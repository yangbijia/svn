...
public class CLients extends JFrame {
	...
	public void initFrame() {
		...
		for (int i = 0; i < table.getModel().getRowCount(); i++) {
			table.getModel().setValueAt("   " + (i + 1), i, 3);
					...
					if (!flag) {
						for (int i = 0; i < table.getRowCount(); i++) {
							if ((boolean) table.getValueAt(i, 2)
									&& "更新".equals(table.getValueAt(i, 1))) {
								// 得到选中的单元格的值，表格中都是字符串
								Object values = table.getValueAt(i, 0);
					...
					if (!flag) {
						for (int i = 0; i < table.getRowCount(); i++) {
							System.out.println("table=" + table + ",value="
									+ (boolean) table.getValueAt(i, 2) + ",i="
									+ i);
							if ((boolean) table.getValueAt(i, 2)
									&& "提交".equals(table.getValueAt(i, 1))) {
								// 得到选中的单元格的值，表格中都是字符串
								Object values = table.getValueAt(i, 0);
								String info = values.toString();
								if (!"".equals(info)) {
